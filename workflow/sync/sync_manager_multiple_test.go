...
func setupMultipleLockManagers(t *testing.T, dbType sqldb.DBType, semaphoreSize int) (context.Context, func(), *Manager, *Manager) {
	...
	syncMgr1 := createLockManager(ctx, info.SessionProxy, &cfg, func(_ context.Context, _ string) (int, QueueingStrategy, error) { return 2, StrictFIFO, nil }, func(key string) {}, WorkflowExistenceFunc)
	require.NotNil(t, syncMgr1)
	require.NotNil(t, syncMgr1.dbInfo.SessionProxy.Session())
	// Second controller
	cfg.ControllerName = "test2"
	syncMgr2 := createLockManager(ctx, info.SessionProxy, &cfg, func(_ context.Context, _ string) (int, QueueingStrategy, error) { return 2, StrictFIFO, nil }, func(key string) {}, WorkflowExistenceFunc)
